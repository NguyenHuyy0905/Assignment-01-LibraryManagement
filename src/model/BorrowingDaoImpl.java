...
public class BorrowingDaoImpl implements BorrowingDao{
    ...
    private final String SQL_GET_BORROWED_BOOK_BY_STUDENT_CODE = "SELECT * FROM books b WHERE b.book_id in (SELECT bb.book_id FROM borrowedbooks bb WHERE bb.student_id in (SELECT s.student_id FROM students s WHERE s.student_code = ?))";
    @Override
